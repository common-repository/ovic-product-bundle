    ...
    class Ovic_Bundle_Woo
    {
        ...
        function custom_data_callback()
        {
            if (isset($_POST['ovic_bundle_ids'])) {
                ...
                $_SESSION['ovic_bundle_ids'] = preg_replace('/[^,\/0-9]/', '', wp_unslash($_POST['ovic_bundle_ids']));
            }
            wp_die();
        }
        ...
        function ovic_bundle_cart_item_name($name, $item)
        {
            if (isset($item['ovic_bundle_parent_id']) && !empty($item['ovic_bundle_parent_id'])) {
                if ((strpos($name, '</a>') !== false) && (get_option('_ovic_bundle_link', 'yes') == 'yes')) {
                    ...
                    return get_the_title($item['ovic_bundle_parent_id']) . ' &rarr; ' . wp_strip_all_tags($name);
                }
            } else {
                return $name;
            }
        ...
        {
            if (isset($_POST['ovic_bundle_ids'])) {
                $ovic_bundle_ids = wp_unslash($_POST['ovic_bundle_ids']);
        ...
        function before_order_itemmeta($item_id)
        {
            if (($ovic_bundle_parent_id = wc_get_order_item_meta($item_id, 'ovic_bundle_parent_id', true))) {
                echo sprintf(
                    ...
                    esc_html__('(bundled in %s)', 'ovic-bundle'),
                    esc_html(get_the_title($ovic_bundle_parent_id))
                );
            }
        }
        ...
        function ovic_bundle_add_to_cart_ids()
        {
            ...
            echo '<input name="ovic_bundle_ids" id="ovic_bundle_ids" type="hidden" value="' . esc_attr($ovic_bundle_ids) . '"/>';
        }
                        ...
                        <tr class="ovic_bundle-product"
                            ...
                            <?php if ($_ovic_bundle_thumb != 'no') { ?>
                                <td class="ovic_bundle-thumb column-thumb">
                                    <div class="thumb">
                                        <?php
                                        $thumbnail = apply_filters('ovic_bundle_item_thumbnail', $ovic_bundle_product->get_image(array(
                                            60,
                                            60
                                        )), $ovic_bundle_product);
                                        echo wp_kses_data($thumbnail);
                                        ?>
                                    </div>
                                </td>
                            <?php } ?>
                            <td class="ovic_bundle-title column-name">
                                ...
                                if (($_ovic_bundle_qty == 'yes') && $ovic_bundle_optional != 'on') {
                                    $qty = apply_filters('ovic_bundle_text_qty', $ovic_bundle_item['qty'] . ' × ', $ovic_bundle_item['qty'], $ovic_bundle_product);
                                    echo wp_kses_data($qty);
                                }
                                ...
                                $item_name = apply_filters('ovic_bundle_item_name', $ovic_bundle_item_name, $ovic_bundle_product);

                                echo wp_kses_data($item_name);
                            ...
                            <?php if ($_ovic_bundle_price != 'no') { ?>
                                <td class="ovic_bundle-price column-price">
                                    <div class="price">
                                        ...
                                        $bundle_price = apply_filters('ovic_bundle_item_price', $ovic_bundle_price, $ovic_bundle_product);

                                        echo wp_kses_data($bundle_price);
                                        ?>
                                    </div>
                                </td>
                            <?php } ?>
                            <?php if ($_ovic_bundle_discount != 'no') { ?>
                                <td class="ovic_bundle-total column-discount">
                                    <div class="discount">
                                        ...
                                        $total_price       = apply_filters('ovic_bundle_item_total', $ovic_bundle_price, $ovic_bundle_product);

                                        echo wp_kses_data($total_price);
                                        ?>
                                    </div>
                                </td>
                            <?php } ?>
