        ...
        function search_products($found_products)
        {
            if (isset($_GET['ovic_bundle_search'])) {
                ...
                if (!empty($found_products)) {
                    ...
                    $html = '<ul><span>'.sprintf(/* translators: %s: number */esc_html__('No results found for "%s"', 'ovic-bundle'), esc_html($_GET['term'])).'</span></ul>';
                }
                ...
                <table>
                    <tr>
                        <th>
                            ...
                            <a href="<?php echo esc_url(admin_url('admin.php?page=ovic-product-bundle')); ?>"
                               target="_blank">
                                            ...
                                            if (is_array($ovic_bundle_items) && count($ovic_bundle_items) > 0) {
                                                foreach ($ovic_bundle_items as $ovic_bundle_item) {
                                                    ...
                                                    if ($ovic_bundle_product->is_type('variable')) {
                                                        echo '<li '.(!$ovic_bundle_product->is_in_stock() ? 'class="out-of-stock"' : '').' data-id="'.esc_attr($ovic_bundle_item_id).'" data-price="'.esc_attr($ovic_bundle_product->get_variation_price('min')).'" data-price-max="'.esc_attr($ovic_bundle_product->get_variation_price('max')).'" data-price-sale="'.esc_attr($ovic_bundle_price_sale).'"><span class="move"></span><span class="qty"><input type="number" value="'.esc_attr($ovic_bundle_item_qty).'" min="0"/></span><span class="sale"><input type="number" value="'.esc_attr($ovic_bundle_item_sale).'" min="0" max="100"/>%</span>  <span class="name">'.esc_attr($ovic_bundle_product->get_name()).'</span> (#'.esc_attr($ovic_bundle_product->get_id()).' - '.esc_attr($ovic_bundle_product->get_price_html()).')<span class="remove">×</span></li>';
                                                    } else {
                                                        echo '<li '.(!$ovic_bundle_product->is_in_stock() ? 'class="out-of-stock"' : '').' data-id="'.esc_attr($ovic_bundle_item_id).'" data-price="'.esc_attr($ovic_bundle_product->get_price()).'" data-price-max="'.esc_attr($ovic_bundle_product->get_price()).'" data-price-sale="'.esc_attr($ovic_bundle_price_sale).'"><span class="move"></span><span class="qty"><input type="number" value="'.esc_attr($ovic_bundle_item_qty).'" min="0"/></span><span class="sale"><input type="number" value="'.esc_attr($ovic_bundle_item_sale).'" min="0" max="100"/>%</span> <span class="name">'.esc_attr($ovic_bundle_product->get_name()).'</span> (#'.esc_attr($ovic_bundle_product->get_id()).' - '.esc_attr($ovic_bundle_product->get_price_html()).')<span class="remove">×</span></li>';
                                                    }
                                                }
                                            }
                                        }
                    ...
                    <tr class="ovic_bundle_tr_space">
                        <th><?php echo esc_html__('Regular price', 'ovic-bundle').' ('.esc_html(get_woocommerce_currency_symbol()).')'; ?></th>
                        <td>
                        ...
                        <td>
                            <div class="w100">
                                <textarea name="ovic_bundle_before_text"
                                          placeholder="<?php esc_html_e('The text before bundled products', 'ovic-bundle'); ?>"><?php echo wp_kses_data(get_post_meta($post_id, 'ovic_bundle_before_text', true)); ?></textarea>
                            </div>
                        </td>
                    </tr>
                    <tr class="ovic_bundle_tr_space">
                        ...
                        <td>
                            <div class="w100">
                                <textarea name="ovic_bundle_after_text"
                                          placeholder="<?php esc_html_e('The text after bundled products', 'ovic-bundle'); ?>"><?php echo wp_kses_data(get_post_meta($post_id, 'ovic_bundle_after_text', true)); ?></textarea>
                            </div>
                        </td>
                    </tr>
                </table>
